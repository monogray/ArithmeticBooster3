...
public class GameCore{
	...
	public void draw(Canvas canvas, boolean _isTouch, int _newX, int _newY, int _diffX, int _diffY) {
		touchProc(_isTouch, _newX, _newY, _diffX, _diffY);
	...
	private void drawGraphic_Stage_1(Canvas canvas) {
		...
		canvas.drawText("Speed: " + MonoMath.round(curBooble.getSpeed(), 2), 10, 44, paint);
	...
	private void startGame() {
		...
		// Set start speed 
		for (int i = 0; i < boobles.length; i++) {
			boobles[i].setSpeed((float) (h * 0.0054)); // 0.0004
		}
	}
